...
import { eventSource, event_types, chat, name1, name2 } from '../../../../script.js';
...
// ---- localStorage-based settings (community style) ----
...
function persistSettings() {
    try {
        localStorage.setItem(LS_KEY, JSON.stringify(_settingsCache || getSettings()));
    } catch (e) {
        console.warn('[iPhone-Sim] Could not save settings:', e);
    }
}
...
// ============================================================
// BOOTSTRAP — jQuery async เหมือน CattaHub / Multiplayer
// ============================================================
jQuery(async () => {
    console.log('[iPhone-Sim] Extension loading...');
    await injectHTML();
    loadFromSettings();
    bindGlobalEvents();
    ...
    // Listen to ST events (เหมือน community style)
    if (eventSource) {
        eventSource.on(event_types.CHAT_CHANGED, () => {
            // refresh friends list when char changes
            if (state.currentApp === 'friends') loadFriendsList();
        });
    }

    console.log('[iPhone-Sim] Loaded ✓');
});
...
async function injectHTML() {
    // ---- FAB ปุ่มลอย (ใช้ appendChild + inline style เพื่อป้องกัน ST override) ----
    const fab = document.createElement('button');
    fab.id = 'iphone-fab';
    fab.title = 'iPhone Simulator';
    fab.innerHTML = `<svg viewBox="0 0 24 24"><path d="M17 2H7C5.34 2 4 3.34 4 5v14c0 1.66 1.34 3 3 3h10c1.66 0 3-1.34 3-3V5c0-1.66-1.34-3-3-3zm-5 18.5c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zm5-4H7V5h10v11.5z"/></svg><span class="notif-dot" id="fab-notif-dot"></span>`;
    fab.style.cssText = `
    ...
    fab.addEventListener('mouseenter', () => {
        ...
        fab.style.boxShadow = '0 6px 28px rgba(0,0,0,0.6)';
    });
    ...
    // SVG inline style
    const fabSvg = fab.querySelector('svg');
    if (fabSvg) fabSvg.style.cssText = 'width:26px;height:26px;fill:#fff;pointer-events:none;';
    document.body.appendChild(fab);
    console.log('[iPhone-Sim] FAB injected ✓', document.getElementById('iphone-fab'));

    // ---- Overlay + iPhone Frame ----
    const overlay = document.createElement('div');
    overlay.id = 'iphone-overlay';
...
`;
    ...
    console.log('[iPhone-Sim] Overlay injected ✓');
}
...
function bindGlobalEvents() {
    // FAB — toggle overlay ด้วย inline style (ป้องกัน ST override class)
    document.getElementById('iphone-fab').addEventListener('click', () => {
    ...
    document.getElementById('iphone-overlay').addEventListener('click', function(e) {
        if (e.target === this) {
...
async function botReply(userText) {
    ...
    try {
        ...
        // name1 = user name, name2 = bot/char name (จาก import script.js)
        const playerName = (typeof name1 !== 'undefined' && name1) ? name1 : 'ผู้ใช้';
...
function loadFriendsList() {
    ...
    // Get bots from SillyTavern context — แบบ CattaHub style
    let bots = [];
    try {
        if (typeof SillyTavern !== 'undefined' && SillyTavern.getContext) {
        ...
        console.warn('[iPhone-Sim] Could not load characters:', e);
    }
...
function renderTweetFeed() {
    ...
    // User avatar + name — ใช้ name1 จาก import + SillyTavern.getContext() แบบ community
    try {
        let userAvatar = '';
        let userName = (typeof name1 !== 'undefined' && name1) ? name1 : 'Me';
        if (typeof SillyTavern !== 'undefined' && SillyTavern.getContext) {
            const ctx = SillyTavern.getContext();
            if (ctx && ctx.user_avatar) userAvatar = `/User Avatars/${ctx.user_avatar}`;
            if (ctx && ctx.name) userName = ctx.name;
        }
        $('#compose-avatar').attr('src', userAvatar);
        $('#profile-avatar').attr('src', userAvatar);
        $('#thread-compose-avatar').attr('src', userAvatar);
        s.userHandle = '@' + userName.toLowerCase().replace(/\s+/g, '_');
        s.userName = userName;
    } catch(e) {}
