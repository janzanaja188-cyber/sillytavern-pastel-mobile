...
/* Nav Bar — X-style */
.isim-nav{
    height:44px;background:rgba(0,0,0,.85);backdrop-filter:blur(20px);
...
/* Chat Screen */
#isim-chat-header{
    padding:8px 14px;background:rgba(0,0,0,.9);backdrop-filter:blur(20px);
    border-bottom:0.5px solid var(--sep);
  ...
  <div id="isim-screen">
    ...
    <div class="isim-screen" id="isim-scr-chat">
      <div class="isim-nav" style="height:auto;min-height:44px;padding:6px 12px;">
        ...
        <button class="isim-nav-action" onclick="window.__isimStartCall()">📞</button>
      </div>
      <div id="isim-chat-header">
        ...
        <div class="isim-chat-tools">
          ...
          <button class="isim-tool-btn" onclick="window.__isimToggleEditMode()">Edit</button>
      ...
      <div id="isim-wp-picker">
        <div style="font-size:11px;color:var(--txt3);margin-bottom:6px;">Chat Wallpaper</div>
        <div style="display:flex;gap:8px;overflow-x:auto;">
          ...
          <div class="isim-wp-option" style="background:linear-gradient(135deg,#0a1a00,#1a3a00);" onclick="window.__isimSetChatWp('gradient4')"></div>
      ...
      <div id="isim-plussheet">
        ...
        <button class="isim-plus-item" onclick="window.__isimToggleWpPicker()"><span>🎨</span><span>Wallpaper</span></button>
      </div>
      ...
      <div id="isim-inputbar">
        ...
        <button id="isim-botsendbtn" onclick="window.__isimNudge()" title="Request bot reply">⬆</button>
        <button id="isim-cancelbtn" onclick="window.__isimCancelBot()" title="Cancel">✕</button>
...
function loadHistory(fid) {
    ...
    else hist.forEach((m,i) => appendBubble(m, false, i));
...
async function botReply() {
    ...
    try {
        ...
        const histText = hist.map(m => {
            const who = m.from === 'user' ? playerName : f.name;
            if (m.type === 'sticker') return `${who}: [Sent a sticker]`;
            if (m.type === 'photo') return `${who}: [Sent a photo]`;
            if (m.type === 'location') return `${who}: [Location: ${m.content}]`;
            if (m.type === 'redenv') return `${who}: [Sent Red Envelope ฿${m.content?.amount}]`;
        ...
        // Sticker meanings for bot
        const stickerInfo = (s.stickers||[]).map((stk,i) => `Sticker ${i}: ${stk.meaning||'image'}`).join(', ');

        // Story context
        const myStories = (s.stories||[]).filter(st => st.isMe && Date.now()-st.timestamp < 24*3600000);
        const storyContext = myStories.length ? `${playerName} has an active story: "${myStories[myStories.length-1].imageMeaning||'image'}"` : '';

        const prompt = `[iPhone iMessage Roleplay]
You are ${f.name}. ${f.persona||'A person chatting via iPhone.'}
You are currently texting ${playerName} via iPhone Messages app.
${userNote ? `Context note from ${playerName}: "${userNote}"` : ''}
${validBotNote ? `Your own note/status: "${validBotNote}"` : ''}
${storyContext}

IMPORTANT OUTPUT FORMAT:
Each message you send MUST start with [MSG] on its own line.
Example of a valid reply with 2 messages:
[MSG] สวัสดีค่ะ ว่าไงบ้าง?
[MSG] คิดถึงเลยนะ!

OPTIONAL ACTIONS (put on their own line, NOT inside [MSG]):
- Send a sticker: [STICKER:0] or [STICKER:1] etc.
- Send emoji sticker: [STICKER_EMOJI:😊]
- Share location: [LOCATION:Siam Paragon Bangkok]
- Send red envelope: [REDENV:100:Happy!]
- Send a voice message: [VOICE:5] (number = seconds)
- Post a note: [NOTE:your note text here]

RULES:
- ALWAYS use [MSG] prefix for every text message
- Write naturally like real Thai/English texting
- 1-4 messages per reply, SHORT like real SMS
- Stay in character as ${f.name}, never say you are AI
- Match language: if ${playerName} writes Thai, reply Thai; if English, reply English
- NO asterisks, NO *actions*, NO quotes around messages
${stickerInfo ? `\nAvailable stickers: ${stickerInfo}` : ''}

Conversation history:
${histText}

Now reply as ${f.name} (use [MSG] for each message):`;
        ...
        // Remove any stray name prefix lines like "Aria:" at start of lines
        rawReply = rawReply.replace(new RegExp(`^${f.name}:\\s*`, 'gm'), '');
        if (!rawReply.trim()) rawReply = '[MSG]สวัสดีค่ะ';
        ...
        // First handle [NOTE] anywhere in the text
        const noteMatches = [...rawReply.matchAll(/\[NOTE:([^\]]+)\]/g)];
        for (const nm of noteMatches) {
            ...
            showNotif(f.name, 'Posted a note: ' + noteText, f.avatar);
            toast(`${f.name} posted a note`);
        }
        // Remove [NOTE:...] from rawReply so it doesn't show as text
        ...
        for (const line of lines) {
            ...
            } else if (line.match(/^\[STICKER:(\d+)\]$/)) {
                const i = parseInt(line.match(/^\[STICKER:(\d+)\]$/)[1]);
                const stk = (cfg().stickers||[])[i];
                if (stk) parts.push({ type: 'sticker_img', src: stk.src, meaning: stk.meaning });
                else parts.push({ type: 'sticker_emoji', content: '😊' });
            } else if (line.match(/^\[STICKER_EMOJI:(.+)\]$/)) {
                parts.push({ type: 'sticker_emoji', content: line.match(/^\[STICKER_EMOJI:(.+)\]$/)[1] });
            } else if (line.match(/^\[LOCATION:(.+)\]$/)) {
                parts.push({ type: 'location', content: line.match(/^\[LOCATION:(.+)\]$/)[1].trim() });
            } else if (line.match(/^\[REDENV:([^:]+):?(.*)\]$/)) {
                const m2 = line.match(/^\[REDENV:([^:]+):?(.*)\]$/);
                parts.push({ type: 'redenv', amount: m2[1].trim(), note: m2[2].trim() });
            } else if (line.match(/^\[VOICE:(\d+)\]$/)) {
                const dur = parseInt(line.match(/^\[VOICE:(\d+)\]$/)[1]) || 3;
                parts.push({ type: 'voice', duration: dur });
            } else if (line && !line.startsWith('[')) {
                // Plain text line without [MSG] prefix — still use it
                parts.push({ type: 'text', content: line });
            }
        }
        if (!parts.length) {
            // Last resort: use whole rawReply as single message
            const cleaned = rawReply.replace(/\[[A-Z_]+:[^\]]*\]/g, '').trim();
            if (cleaned) parts = [{ type: 'text', content: cleaned }];
            else parts = [{ type: 'text', content: 'สวัสดีค่ะ' }];
        }
        ...
        for (let i = 0; i < parts.length; i++) {
            ...
            const part = parts[i];
            const hist2 = cfg().history[f.id] || [];
            const newIdx = hist2.length;
            let botMsg;
            if (part.type === 'sticker_img') {
                botMsg = { from: 'bot', type: 'sticker', content: part.src, time: now() };
                saveMsg(botMsg);
                appendBubble(botMsg, true, newIdx);
                showNotif(f.name, 'Sent a sticker', f.avatar);
                ...
                saveMsg(botMsg);
                showNotif(f.name, `${part.content}`, f.avatar);
            } else if (part.type === 'location') {
                botMsg = { from: 'bot', type: 'location', content: part.content, time: now() };
                saveMsg(botMsg);
                appendBubble(botMsg, true, newIdx);
                showNotif(f.name, `Shared location: ${part.content}`, f.avatar);
                ...
                if (amt > 0) {
                    ...
                    cfg().bank.transactions.unshift({ type:'receive', amount: amt, from: f.name, note: part.note || 'Red Envelope', date: Date.now() });
                    save(); renderBank();
                }
                showNotif(f.name, `Sent you a Red Envelope ฿${part.amount}`, f.avatar);
            } else if (part.type === 'voice') {
                botMsg = { from: 'bot', type: 'voice', content: '', duration: part.duration, time: now() };
                saveMsg(botMsg);
                appendBubble(botMsg, true, newIdx);
                showNotif(f.name, `Sent a voice message (${part.duration}s)`, f.avatar);
            } else {
...
Write ONLY the note text. No quotes. No tags. Just the note. Example: "Having a great day at the beach!"`;
        ...
        reply = cleanThink(String(reply||'').trim()).replace(/^["']|["']$/g,'');
...
window.__isimStartCall = function() {
    ...
    if (botNmEl) botNmEl.textContent = `${f.name} is speaking...`;
    ...
    setTimeout(() => {
        ...
        showCallFloat(`Hello! It's ${activeFriend?.name}.`);
    }, 1500);
};
...
window.__isimCallSend = async function() {
    ...
    try {
        ...
        const prompt = `[Phone call roleplay]
You are ${activeFriend.name}. ${activeFriend.persona||''}
You are on a live phone call with ${getUserName()}.
Recent chat context:
${hist}

${getUserName()} just said: "${text}"

Reply with 1-2 natural spoken sentences. No asterisks. No <think> tags. No quotation marks. Just what ${activeFriend.name} says aloud. Match language (Thai if Thai input, English if English).
${activeFriend.name}:`;
        let reply = '';
        if (typeof ctx.generateQuietPrompt === 'function') {
            reply = await ctx.generateQuietPrompt(prompt, false, false);
        ...
        // Remove any [MSG] artifacts
        reply = reply.replace(/\[MSG\]/g,'').replace(/\[[A-Z_]+:[^\]]*\]/g,'').trim();
        if (!reply) reply = 'ได้ยินนะ ว่าไงต่อ?';
...
window.__isimSubmitDetailReply = async function() {
    ...
    setTimeout(async () => {
        try {
            ...
            const replyPrompt = `You are ${botName}. ${botPersona}
Someone just replied to this tweet: "${t.body}"
Their reply: "${text}"
Write a SHORT natural reply (1 sentence max, under 15 words). No hashtags. No emojis unless natural. Match the language used.
${botName}:`;
            let botReplyText = '';
            if (typeof ctx.generateQuietPrompt === 'function') {
            ...
            if (botReplyText.startsWith(botName+':')) botReplyText = botReplyText.slice(botName.length+1).trim();
            if (!botReplyText) botReplyText = 'Interesting!';
            ...
            // fallback
            const fallbacks = ['Interesting!','Great point!','Totally agree.','Thanks!'];
            t.replies_list.push({ user: t.user, body: fallbacks[Math.floor(Math.random()*fallbacks.length)] });
            t.replies++;
            cfg().tweets = tweets; save();
            window.__isimOpenTweet(currentOpenTweetId);
            renderTweetFeed();
...
React to this tweet in 1 short sentence (under 12 words). Natural, casual tone.
Tweet: "${text}"
${botName}:`;
            if (typeof ctx.generateQuietPrompt === 'function') {
            ...
            if (botComment.startsWith(botName+':')) botComment = botComment.slice(botName.length+1).trim();
        } catch {}
        if (!botComment) { const f=['Nice!','Relatable!','Agreed.','Love it!']; botComment=f[Math.floor(Math.random()*f.length)]; }
    ...
    <div class="isim-st-panel">
      <div class="inline-drawer">
        ...
        <div class="inline-drawer-content">
          ...
          <hr><small style="color:#888">v3.0.0</small>
        </div>
      </div>
    </div>`;
...
jQuery(async () => {
    ...
    console.log('[iPhone-Sim] v3.1.0 loaded');
});
