...
function appendBubble(msg, scroll=true, idx) {
    ...
    if (msg.type === 'sticker') {
        ...
        inner = `<div style="font-size:14px">📍 ${esc(msg.content)}</div>`;
    } else if (msg.type === 'redenv') {
        const c = msg.content;
        inner = `<div style="text-align:center;padding:4px 0">🧧<br><b style="font-size:20px">฿${esc(c.amount)}</b>${c.note ? '<br><span style="font-size:12px;opacity:.7">'+esc(c.note)+'</span>' : ''}</div>`;
    } else {
...
You are texting ${playerName} via iPhone Messages.
${userNote ? `Note from ${playerName}: "${userNote}"` : ''}
${validBotNote ? `Your current profile note: "${validBotNote}"` : ''}
${storyContext}

RULES:
- Reply naturally like real texting
- Send 1-3 short messages, each on its own line starting with [MSG]
- You can send a sticker from your collection: [STICKER:number] where number is 0,1,2...
- You can send a photo sticker (big emoji image): [STICKER_EMOJI:emoji]
- You can send your location: [LOCATION:place name here]
- You can send a red envelope: [REDENV:amount:message]
- You can post a note visible to ${playerName}: [NOTE:your note text]
- Stay in character, never mention AI
- Language: match what ${playerName} uses (Thai/English)

${stickerInfo ? `Available stickers: ${stickerInfo}` : ''}
...
Reply as ${f.name}:`;
        ...
        for (const line of lines) {
            if (line.startsWith('[MSG]')) {
                ...
                else parts.push({ type: 'text', content: '[Sticker]' });
                ...
                const m = line.match(/^\[REDENV:([^:]+):?(.*)\]$/);
                parts.push({ type: 'redenv', amount: m[1].trim(), note: m[2].trim() });
            } else if (line.match(/^\[NOTE:(.+)\]$/)) {
                const noteText = line.match(/^\[NOTE:(.+)\]$/)[1].trim();
                ...
                // plain text — split on [MSG] inline
                const msgParts = line.split('[MSG]').map(p=>p.trim()).filter(Boolean);
                for (const p of msgParts) parts.push({ type: 'text', content: p });
            }
        }
        if (!parts.length) parts = [{ type: 'text', content: rawReply.replace(/\[NOTE:[^\]]+\]/g,'').trim() || '...' }];
...
Context: ${hist}
Write ONLY the note text. No quotes. No tags. Just the note.`;
        const ctx = SillyTavern.getContext();
        let reply = '';
        if (typeof ctx.generateQuietPrompt === 'function') {
            reply = await ctx.generateQuietPrompt(prompt, false, false, '', true);
        } else if (typeof ctx.generate === 'function') {
            reply = await ctx.generate(prompt, { quiet: true });
        }
...
window.__isimCallSend = async function() {
    ...
    try {
        const ctx = SillyTavern.getContext();
        const prompt = `You are ${activeFriend.name}. ${activeFriend.persona||''} You are on a phone call with ${getUserName()}. Reply in 1-2 short sentences. No asterisks. No <think> tags. Natural speech only.
${getUserName()} said: "${text}"
${activeFriend.name}:`;
        let reply = '';
        if (typeof ctx.generateQuietPrompt === 'function') {
            reply = await ctx.generateQuietPrompt(prompt, false, false, '', true);
...
${botName}:`;
            let botReplyText = '';
            if (typeof ctx.generateQuietPrompt === 'function') {
                botReplyText = await ctx.generateQuietPrompt(replyPrompt, false, false, '', true);
            }
...
${botName}:`;
            if (typeof ctx.generateQuietPrompt === 'function') {
                botComment = await ctx.generateQuietPrompt(p2, false, false, '', true);
            }
...
jQuery(async () => {
    ...
    console.log('[iPhone-Sim] v3.0.0 loaded');
});
