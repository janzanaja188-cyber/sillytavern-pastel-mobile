...
<div id="isim-frame" class="dark">
  ...
  <div id="isim-screen">
    ...
    <div class="isim-screen" id="isim-scr-chat">
      ...
      <div id="isim-gen-bar" style="display:flex;align-items:center;gap:8px;padding:6px 12px 8px;background:var(--bg);border-top:0.5px solid var(--sep);">
        <button id="isim-cancelbtn" onclick="window.__isimCancelBot()" title="Cancel" style="display:none;background:var(--bg3);color:var(--txt2);border:none;border-radius:20px;padding:8px 16px;font-size:13px;cursor:pointer;">✕ Cancel</button>
        <button id="isim-botsendbtn" onclick="window.__isimNudge()" title="Generate bot reply" style="flex:1;background:var(--accent);color:#fff;border:none;border-radius:20px;padding:9px 16px;font-size:14px;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;">
          <span>✦</span> Generate Reply
        </button>
        <button onclick="window.__isimToggleEditMode()" style="background:var(--bg3);color:var(--txt2);border:none;border-radius:20px;padding:8px 14px;font-size:13px;cursor:pointer;">Edit</button>
        <button onclick="window.__isimRetry()" style="background:var(--bg3);color:var(--txt2);border:none;border-radius:20px;padding:8px 14px;font-size:13px;cursor:pointer;">↻</button>
        <button onclick="window.__isimNav('call-history')" style="background:var(--bg3);color:var(--txt2);border:none;border-radius:20px;padding:8px 12px;font-size:13px;cursor:pointer;">📋</button>
      </div>
      <div class="isim-home-bar"></div>
    </div>
...
async function botReply() {
    ...
    try {
        ...
        // Build injection prompt — optimized from Chinese reference (index__4_.js)
        const stickerExamples = (cfg().stickers||[]).slice(0,3).map((s,i)=>`[STICKER:${i}] (${s.meaning||'image'})`).join(', ');
        const displayName = f.customName || f.name;
        // Get character context from SillyTavern if available
        let cardDesc = '', cardPersonality = '', cardScenario = '', cardMesExample = '';
        try {
            const ctxData = ctx.characters?.[ctx.characterId] || {};
            cardDesc = ctxData.description || '';
            cardPersonality = ctxData.personality || '';
            cardScenario = ctxData.scenario || '';
            cardMesExample = ctxData.mes_example || '';
        } catch {}

        const injectPrompt = `[Phone Message Mode — Highest Priority]
You are ${displayName}${cardPersonality ? ` (${cardPersonality.substring(0,100)})` : ''} replying to ${playerName} via iPhone text message.
${cardScenario ? `Scenario: ${cardScenario.substring(0,200)}` : ''}
${userNote ? `Note about ${playerName}: "${userNote}"` : ''}
${validBotNote ? `${displayName}'s current status note: "${validBotNote}"` : ''}
${storyContext || ''}
${cardMesExample ? `Example dialogue style:
${cardMesExample.substring(0,300)}` : ''}

Recent conversation:
${histText}

RULES (mandatory):
1. Reply as ${displayName} in 1-3 short text messages. Match the language being used (Thai→Thai, English→English).
2. Output ONLY the reply text. NO asterisks, NO action text, NO roleplay narration, NO quotes.
3. Do NOT prefix with character name.
4. Optional special commands (on their own line if used naturally):
   [STICKER:N] — send saved sticker${stickerExamples ? ' (available: '+stickerExamples+')' : ''}
   [STICKER_EMOJI:emoji] — send emoji sticker
   [LOCATION:place name] — share location  
   [REDENV:amount:message] — send red envelope
   [VOICE:seconds] — send voice note
   [NOTE:text] — post a visible note (24h)
5. Separate multiple messages with a line break only. Do not number them.
...
Reply now as ${displayName}:`;
        ...
        if (typeof ctx.generateQuietPrompt === 'function') {
            rawReply = await ctx.generateQuietPrompt(injectPrompt, false, false);
        } else if (typeof window.generateQuietPrompt === 'function') {
            rawReply = await window.generateQuietPrompt(injectPrompt, false, false);
        } else {
            throw new Error('generateQuietPrompt not available');
        }

        rawReply = cleanThink(rawReply);
        // Strip name prefix anywhere in the text (model often adds it on every line)
        const _nameRx = new RegExp('^' + f.name.replace(/[.*+?^${}()|[\]\\]/g,'\\$&') + '\\s*:\\s*', 'gim');
        rawReply = rawReply.replace(_nameRx, '');
        ...
        // ── Parse bot output ── accept [MSG], plain lines, and special commands
        const lines = rawReply.split('\n').map(l => l.trim()).filter(l => l);
        let parts = [];
        for (const line of lines) {
            if (line.match(/^\[STICKER:(\d+)\]$/i)) {
                const i = parseInt(line.match(/^\[STICKER:(\d+)\]$/i)[1]);
            ...
            } else if (line.match(/^\[STICKER_EMOJI:(.+)\]$/i)) {
            ...
            } else if (line.match(/^\[REDENV:([^:]+):?(.*)\]$/i)) {
                const m2 = line.match(/^\[REDENV:([^:]+):?(.*)\]$/i);
                parts.push({ type: 'redenv', amount: m2[1].trim(), note: m2[2].trim() });
            } else if (line.match(/^\[VOICE:(\d+)\]$/i)) {
                const dur = parseInt(line.match(/^\[VOICE:(\d+)\]$/i)[1]) || 3;
                parts.push({ type: 'voice', duration: dur });
            } else {
                // Accept [MSG]text, "Name: text", or plain text — strip prefix
                let content = line;
                content = content.replace(/^\[MSG\]\s*/i, '');
                const namePrefix = new RegExp('^' + f.name.replace(/[.*+?^${}()|[\]\\]/g,'\\$&') + '\\s*:\\s*', 'i');
                content = content.replace(namePrefix, '');
                content = content.replace(/^["""«»]|["""«»]$/g, '').trim();
                if (content && !content.match(/^\[[A-Z_]+:/i)) {
                    parts.push({ type: 'text', content });
                }
            }
        }
        if (!parts.length) {
            // rawReply had no parseable lines — use the whole thing stripped of tags
            const cleaned = rawReply
                .replace(/\[[A-Z_]+:[^\]]*\]/gi, '')
                .replace(new RegExp(f.name + '\\s*:', 'gi'), '')
                .replace(/^["""«»]|["""«»]$/g, '')
                .trim();
            if (cleaned) {
                // Split by newline or sentence boundary into ≤3 parts
                const sentences = cleaned.split(/(?<=[.!?ๆ…]\s)|\n/).map(s=>s.trim()).filter(s=>s);
                for (const s of sentences.slice(0,3)) parts.push({ type:'text', content: s });
            }
        }
        // Final guard — should never fire but prevents silent empty reply
        if (!parts.length) parts = [{ type: 'text', content: rawReply.trim() || '...' }];
