    ...
    <div id="isim-call">
      ...
      <div class="isim-call-ctls">
        <button class="icircle" onclick="window.__isimMute()">
          <div class="icircle-bg" id="isim-mute-circle">M</div><span style="font-size:12px;color:#fff;">Mute</span>
        </button>
        <button class="icircle" onclick="window.__isimEndCall()">
          <div class="icircle-bg red">End</div><span style="font-size:12px;color:#fff;">End</span>
        </button>
        <button class="icircle">
          <div class="icircle-bg">Spk</div><span style="font-size:12px;color:#fff;">Speaker</span>
        </button>
      </div>
    </div>
...
window.__isimSubmitDetailReply = async function() {
    ...
    setTimeout(() => {
        const botReplies = ['Interesting!', 'I agree with that.', 'Thanks for sharing!', 'Good point!', 'Noted!'];
...
window.__isimPostTweet = function() {
    ...
    setTimeout(() => {
        ...
        if (!t2) return;
        t2.likes += Math.floor(Math.random()*10)+1;
        t2.retweets += Math.floor(Math.random()*3);
        const autoR = ['Nice post!','Relatable!','Agreed.','Save this.','Love it'];
        t2.replies_list.push({ user:'follower_'+Math.floor(Math.random()*999), body:autoR[Math.floor(Math.random()*autoR.length)] });
        t2.replies++;
        cfg().tweets = tweets; save(); renderTweetFeed();
    }, 3000);
};
