...
async function botReply() {
    ...
    try {
        ...
        // Build injection prompt that appends to ST's existing character context
        const injectPrompt = `[iPhone iMessage — ส่วนเสริม]
ตอนนี้ ${f.name} กำลังคุยกับ ${playerName} ผ่านแอป iPhone Messages
${userNote ? `Note จาก ${playerName}: "${userNote}"` : ''}
${validBotNote ? `Note ของ ${f.name}: "${validBotNote}"` : ''}
${storyContext ? storyContext : ''}

ประวัติการสนทนาทาง iPhone:
${histText}

FORMAT การตอบ (บังคับ):
- ทุกข้อความต้องขึ้นต้นด้วย [MSG] เสมอ
- ตอบ 1-4 ข้อความสั้นๆ เหมือนส่ง SMS จริงๆ
- ห้ามใส่ * หรือ action ใดๆ
- ตอบภาษาเดียวกับที่ ${playerName} ใช้

OPTIONAL (ใส่แยกบรรทัด ไม่ใช่ใน [MSG]):
[STICKER:0] หรือ [STICKER_EMOJI:😊] — ส่งสติกเกอร์
[LOCATION:ชื่อสถานที่] — ส่งตำแหน่ง
[REDENV:จำนวน:ข้อความ] — ส่งซองแดง
[VOICE:วินาที] — ส่งเสียง
[NOTE:ข้อความ] — โพสต์โน้ต
${stickerInfo ? `สติกเกอร์ที่มี: ${stickerInfo}` : ''}

ตัวอย่างการตอบ:
[MSG] ว่าไงบ้างอ่ะ?
[MSG] คิดถึงเลยนะ!

ตอบเป็น ${f.name} เดี๋ยวนี้เลย:`;
