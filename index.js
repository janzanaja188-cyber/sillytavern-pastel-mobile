...
async function botReply() {
    ...
    try {
        ...
        // Build injection prompt — keep it concise so ST model cooperates
        const stickerExamples = (cfg().stickers||[]).slice(0,3).map((s,i)=>`[STICKER:${i}] (${s.meaning||'image'})`).join(', ');
        const injectPrompt = `[iPhone Messages — System Note]
You are ${f.name} texting ${playerName} on an iPhone.
${userNote ? `Note about ${playerName}: "${userNote}"` : ''}
${validBotNote ? `${f.name}'s own note: "${validBotNote}"` : ''}
${storyContext || ''}
...
INSTRUCTIONS:
- Reply as ${f.name} in 1–3 short messages like real texting.
- Write ONLY the reply text. No asterisks, no actions, no quotes.
- Match the language ${playerName} is using (Thai if Thai, English if English).
- You MAY optionally add special lines (on their own line, after text messages):
  [STICKER:N] to send a saved sticker  ${stickerExamples ? '— available: ' + stickerExamples : ''}
  [STICKER_EMOJI:emoji] to send an emoji sticker
  [LOCATION:place name] to share your location
  [REDENV:amount:message] to send a red envelope
  [VOICE:seconds] to send a voice note
  [NOTE:text] to post a note (visible 24h)
- Use them ONLY if contextually natural. Do NOT invent sticker numbers that don't exist.
