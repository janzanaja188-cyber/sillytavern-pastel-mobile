...
/* Plus sheet */
#isim-plussheet{display:none;flex-direction:row;flex-wrap:wrap;gap:12px;padding:12px;background:var(--bg2);border-top:0.5px solid var(--sep);flex-shrink:0;justify-content:center;}
#isim-plussheet.show{display:flex;}
.isim-plus-item{display:flex;flex-direction:column;align-items:center;gap:4px;background:var(--bg3);border:none;border-radius:14px;padding:12px 14px;cursor:pointer;font-size:22px;min-width:60px;}
.isim-plus-item span{font-size:11px;color:var(--txt2);}

/* Red envelope sheet */
#isim-redenv-sheet{display:none;position:absolute;bottom:0;left:0;right:0;background:var(--bg2);border-radius:20px 20px 0 0;padding:20px 16px;z-index:50;box-shadow:0 -4px 20px rgba(0,0,0,.3);}
#isim-redenv-sheet.show{display:block;}

/* Sticker tray */
#isim-sticker-tray{display:none;flex-direction:row;flex-wrap:wrap;gap:8px;padding:10px;background:var(--bg2);border-top:0.5px solid var(--sep);max-height:130px;overflow-y:auto;flex-shrink:0;}
#isim-sticker-tray.show{display:flex;}
.isim-sticker-thumb{width:56px;height:56px;object-fit:contain;border-radius:8px;cursor:pointer;background:var(--bg3);}

/* Plus/sticker buttons */
#isim-plusbtn{width:30px;height:30px;border-radius:50%;border:none;background:var(--bg3);color:var(--txt);font-size:18px;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;}
#isim-stickerbtn{width:30px;height:30px;border-radius:50%;border:none;background:var(--bg3);color:var(--txt);font-size:16px;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;}

/* Call float — ย้ายมาใต้ชื่อ */
#isim-call-float{position:relative;z-index:2;min-height:50px;text-align:center;padding:0 16px 8px;}

/* Call screen layout fix */
.isim-call-mid{position:relative;z-index:2;flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;}
  ...
  <div id="isim-screen">
    ...
    <div class="isim-screen" id="isim-scr-chat">
      ...
      <div id="isim-redenv-sheet">
        ...
        <div style="font-size:15px;font-weight:600;color:var(--txt);text-align:center;margin-bottom:12px">Red Envelope</div>
        ...
        <input id="isim-redenv-note" type="text" placeholder="Message (optional)" style="width:100%;box-sizing:border-box;padding:10px;border-radius:10px;border:0.5px solid var(--sep);background:var(--inp);color:var(--txt);font-size:15px;margin-bottom:12px">
        <button onclick="window.__isimSendRedEnvelope()" style="width:100%;padding:12px;border-radius:12px;border:none;background:#e02020;color:#fff;font-size:15px;font-weight:600;cursor:pointer">Send 🧧</button>
        <button onclick="window.__isimCloseRedEnvelope()" style="width:100%;padding:10px;border-radius:12px;border:none;background:var(--bg3);color:var(--txt);font-size:14px;cursor:pointer;margin-top:6px">Cancel</button>
      </div>
      <!-- Sticker picker -->
      ...
      <div id="isim-inputbar">
        <button id="isim-plusbtn" onclick="window.__isimTogglePlus()" title="More">＋</button>
        <button id="isim-stickerbtn" onclick="window.__isimToggleStickerTray()" title="Stickers">🖼</button>
        <textarea id="isim-input" placeholder="iMessage" rows="1"></textarea>
        <button id="isim-nudgebtn" onclick="window.__isimNudge()" title="Send 2 (nudge bot)">⤴</button>
    ...
    <div class="isim-screen" id="isim-scr-settings">
      ...
      <div style="flex:1;overflow-y:auto;">
        <div class="isim-set-section">
          ...
          <div class="isim-set-group">
            ...
            <div class="isim-set-row" style="cursor:pointer" onclick="document.getElementById('isim-bg-input').click()">
              <span>Chat Wallpaper</span><span style="color:var(--txt3)">›</span>
            </div>
            <div class="isim-set-row" style="cursor:pointer" onclick="window.__isimResetBg()">
              <span>Remove Wallpaper</span><span style="color:#ff375f">✕</span>
            </div>
          </div>
        </div>
        <div class="isim-set-section">
        ...
        <div class="isim-set-section">
          ...
          <div class="isim-set-group">
            <div class="isim-set-row" style="cursor:pointer" onclick="window.__isimClearChat()">
              <span style="color:#ff375f">Clear All Chats</span><span style="color:var(--txt3)">›</span>
            </div>
            <div class="isim-set-row" style="cursor:pointer" onclick="window.__isimClearFriends()">
              <span style="color:#ff375f">Remove All Contacts</span><span style="color:var(--txt3)">›</span>
            </div>
      ...
      <input type="file" id="isim-bg-input" accept="image/*" style="display:none" onchange="window.__isimSetBg(this)">
    </div>
...
function appendBubble(msg, scroll = true) {
    ...
    if (msg.type === 'sticker') {
        ...
        const c = msg.content;
        inner = `<div style="text-align:center;padding:4px 0">🧧<br><b style="font-size:18px">${esc(c.amount)} บาท</b>${c.note ? '<br><span style="font-size:11px;opacity:.7">'+esc(c.note)+'</span>' : ''}</div>`;
    } else {
...
// ---- Bot Reply (ใช้ ST generateQuietPrompt — ไม่มี CORS บนมือถือ) ----
async function botReply(userText) {
    ...
    typingRow.innerHTML = `<img class="isim-av" src="${esc(activeFriend.avatar||'')}" alt="" onerror="this.style.visibility='hidden'"><div class="isim-typing"><span></span><span></span><span></span></div>`;
    ...
    try {
        ...
        // สร้าง history string
        const hist = (s.history[f.id] || []).slice(-10);
        const histText = hist.map(m =>
            (m.from === 'user' ? playerName : f.name) + ': ' + m.content
        ).join('\n');

        // Prompt ที่ส่งให้ ST generate
        const prompt = `[iPhone SMS Simulation]
You are ${f.name}. ${f.persona || ''}
You are texting ${playerName} via iPhone.
${s.notes[f.id] ? `Context note: ${s.notes[f.id]}` : ''}
${s.botnotes[f.id] ? `Instruction: ${s.botnotes[f.id]}` : ''}
Rules: Reply in Thai naturally. Short like real SMS (1-3 sentences). No emojis. Stay in character. Never say you are AI.

Chat history:
${histText}
${userText ? playerName + ': ' + userText : '(nudge — reply naturally)'}
${f.name}:`;

        // ใช้ ST generateQuietPrompt (ให้ ST เจนให้ ไม่ต้องเรียก API เอง)
        const ctx = SillyTavern.getContext();
        let reply = '';
        ...
        if (typeof ctx.generateQuietPrompt === 'function') {
            ...
            // fallback: inject เข้า ST chat แล้วอ่านกลับมา
            throw new Error('generateQuietPrompt not available');
        }

        // ตัด prefix ถ้า model ตอบกลับมาพร้อมชื่อ
        reply = String(reply || '').trim();
        if (reply.startsWith(f.name + ':')) reply = reply.slice(f.name.length + 1).trim();
        if (!reply) reply = '...';
        ...
        isTyping = false;
        if (sendBtn) sendBtn.disabled = false;
        // แสดง error จริงๆ ใน toast ไม่ใช่ในแชท
...
window.__isimStartCall = function() {
    ...
    const callStart = now();
    callLog('📞 โทรหา ' + f.name + ' เวลา ' + callStart);
    setTimeout(() => {
        if (!callActive) return;
        if (stat) stat.textContent = 'Connected';
        callLog('✅ เชื่อมต่อกับ ' + f.name + ' แล้ว');
...
window.__isimEndCall = function() {
    ...
    const m = Math.floor(callTimer/60), s = callTimer%60;
    callLog('📵 วางสาย — นาน ' + m + ':' + String(s).padStart(2,'0') + ' น. ' + now());
    toast('Call ended');
};
...
function renderStickerTray() {
    ...
    if (!stickers.length) {
        t.innerHTML = '<div style="padding:16px;text-align:center;color:var(--txt3);font-size:13px">No stickers yet — tap + then Sticker to add</div>';
        return;
    }
...
window.__isimSendLocation = function() {
    ...
    if (navigator.geolocation) {
        ...
        navigator.geolocation.getCurrentPosition(
            p => send(p.coords.latitude.toFixed(4) + ', ' + p.coords.longitude.toFixed(4)),
            () => send('Bangkok, Thailand')
        );
    } else send('Bangkok, Thailand');
};
...
window.__isimSendRedEnvelope = function() {
    ...
    appendBubble(msg); saveMsg(msg);
    botReply('[ผู้ใช้ส่งอั่งเปา ' + amt + ' บาท' + (note ? ' หมายเหตุ: ' + note : '') + ']');
};

// ---- Wallpaper ----
window.__isimSetBg = function(input) {
    const f = input.files[0]; if (!f) return;
    const r = new FileReader();
    r.onload = e => {
        cfg().wallpaper = e.target.result;
        save(); applyWallpaper(); toast('Wallpaper set!');
    };
    r.readAsDataURL(f);
    input.value = '';
};
window.__isimResetBg = function() {
    cfg().wallpaper = '';
    save(); applyWallpaper(); toast('Wallpaper removed');
};
function applyWallpaper() {
    const msgs = document.getElementById('isim-msgs');
    if (!msgs) return;
    const wp = cfg().wallpaper;
    msgs.style.backgroundImage = wp ? `url(${wp})` : '';
    msgs.style.backgroundSize = wp ? 'cover' : '';
}

// ---- Settings data actions ----
window.__isimClearChat = function() {
    if (!confirm('ลบแชทของ ' + (activeFriend?.name || 'ทุกคน') + '?')) return;
    const fid = activeFriend?.id;
    if (fid) { cfg().history[fid] = []; save(); loadHistory(fid); toast('Chat cleared'); }
    else { cfg().history = {}; save(); toast('All chats cleared'); }
};
window.__isimClearFriends = function() {
    if (!confirm('ลบผู้ติดต่อทั้งหมด?')) return;
    cfg().friends = []; activeFriend = null; save(); toast('Contacts removed');
};
window.__isimResetAll = function() {
    if (!confirm('รีเซ็ตทุกอย่าง? (ข้อมูลจะหายหมด)')) return;
    localStorage.removeItem('isim_v1');
    _cfg = null; activeFriend = null;
...
function callLog(msg) {
    ...
    if (!msgs) return;
    msgs.insertAdjacentHTML('beforeend',
        `<div class="isim-sys" style="color:var(--txt3);font-size:11px;text-align:center;padding:4px 0">📞 ${esc(msg)}</div>`);
    msgs.scrollTop = msgs.scrollHeight;
}

// ---- appendBubble: support sticker, photo, location, redenv ----
