...
// ── Media Processing (smart-media-assistant adapted) ──
let _mediaCfg={imageQuality:85,maxImageDimension:2048,maxFileSize:20};
async function _processImageFile(file){
    if(!file||!file.type.startsWith('image/'))throw new Error('Not an image');
    return new Promise((res,rej)=>{
        const img=new Image(),canvas=document.createElement('canvas'),ctx2d=canvas.getContext('2d'),objUrl=URL.createObjectURL(file);
        img.onload=async()=>{try{let{width,height}=img;const maxD=_mediaCfg.maxImageDimension;
            if(width>maxD||height>maxD){if(width>=height){height=Math.round(height*maxD/width);width=maxD;}else{width=Math.round(width*maxD/height);height=maxD;}}
            canvas.width=width;canvas.height=height;ctx2d.drawImage(img,0,0,width,height);
            const dataUrl=canvas.toDataURL('image/jpeg',_mediaCfg.imageQuality/100);const b64=dataUrl.split(',')[1];
            let finalUrl=dataUrl;
            for(const p of['../../../../scripts/utils.js','../../../utils.js']){try{const mod=await import(p);if(typeof mod.saveBase64AsFile==='function'){const uid=Date.now()+'_'+Math.random().toString(36).slice(2,7);const saved=await mod.saveBase64AsFile(b64,'phone',uid,'jpg');if(saved){finalUrl=saved;break;}}}catch(_){}}
            URL.revokeObjectURL(objUrl);res({url:finalUrl,b64});
        }catch(e){URL.revokeObjectURL(objUrl);rej(e);}};
        img.onerror=()=>{URL.revokeObjectURL(objUrl);rej(new Error('Load failed'));};img.src=objUrl;
    });
}
async function _sendToSTChat(text){
    const safe=String(text||'').replaceAll('|','¦');const cmd='/send '+safe+' | /trigger';
    for(const p of['/scripts/slash-commands.js','../../scripts/slash-commands.js','../../../../scripts/slash-commands.js']){try{const mod=await import(p);if(typeof mod.executeSlashCommandsWithOptions==='function'){await mod.executeSlashCommandsWithOptions(cmd,{handleParserErrors:true,handleExecutionErrors:true,source:'iphone-sim'});return true;}}catch(_){}}
    if(typeof window.triggerSlash==='function'){window.triggerSlash(cmd);return true;}return false;
}
// ────────────────────────────────────────────────────
...
.isim-chat-row:active{background:rgba(255,255,255,.05);}
.isim-chat-row::after{
    content:'';position:absolute;left:79px;right:0;bottom:0;
...
.isim-chat-online{
    width:14px;height:14px;background:#30d158;border-radius:50%;
    position:absolute;bottom:1px;right:1px;border:2.5px solid var(--bg);
}
...
.isim-chat-preview{font-size:14px;color:var(--txt2);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.isim-chat-unread{
...
.isim-chat-pin{font-size:13px;color:var(--txt3);flex-shrink:0;}
/* Swipe action hint */
.isim-chat-row .isim-swipe-del{
    display:none;
}

/* ============================================
   CHAT SCREEN — iOS Messages style
   ============================================ */
#isim-chat-screen{
    display:flex;flex-direction:column;flex:1;overflow:hidden;
    background:var(--bg);position:absolute;inset:0;z-index:15;
    transform:translateX(100%);transition:transform .3s cubic-bezier(.25,.46,.45,.94);
}
#isim-chat-screen.open{transform:translateX(0);}

/* Blurred header */
#isim-chat-header{
    padding:0 12px 8px;
    background:rgba(0,0,0,.82);backdrop-filter:blur(28px) saturate(180%);
...
.isim-back-btn{
    background:none;border:none;color:var(--accent);
    font-size:15px;cursor:pointer;display:flex;align-items:center;gap:3px;
...
.isim-chat-name{font-size:13px;font-weight:600;color:var(--txt);}
.isim-chat-status{font-size:11px;color:var(--accent);}
.isim-chat-tools{display:flex;gap:2px;flex-shrink:0;}
.isim-tool-btn{
    background:none;border:none;border-radius:10px;
    padding:6px 9px;font-size:13px;color:var(--txt2);cursor:pointer;
    font-weight:500;
}
.isim-tool-btn:active{background:var(--bg3);}
.isim-tool-sep{width:0.5px;background:var(--sep);margin:4px 0;}
...
#isim-msgs::-webkit-scrollbar{width:0;}

/* Date separator */
.isim-date-sep{
    text-align:center;font-size:11px;color:var(--txt3);
    padding:14px 0 4px;font-weight:500;letter-spacing:.2px;
}
...
.isim-row.out{align-self:flex-end;flex-direction:row-reverse;max-width:78%;}
.isim-row.in{align-self:flex-start;}
/* Consecutive messages: smaller gap */
.isim-row.in+.isim-row.in,
.isim-row.out+.isim-row.out{ margin-top:1px; }
.isim-row.in+.isim-row.out,
.isim-row.out+.isim-row.in{ margin-top:8px; }
...
/* Incoming bubble — dark with tail bottom-left */
.isim-row.in .isim-bub{
    background:var(--bg3);
    color:var(--txt);
...
.isim-row.in+.isim-row.in .isim-bub{
    border-radius:18px;border-bottom-left-radius:18px;
}
/* Outgoing bubble — accent color with tail bottom-right */
.isim-row.out .isim-bub{
    background:var(--bub-out);
    color:#fff;
    border-bottom-right-radius:5px;
    box-shadow:0 1px 4px rgba(29,155,240,.3);
}
.isim-row.out+.isim-row.out .isim-bub{
    border-radius:18px;border-bottom-right-radius:18px;
}

/* Timestamp */
.isim-time{font-size:10px;color:var(--txt3);padding:0 2px;align-self:flex-end;white-space:nowrap;}
.isim-seen{font-size:10px;color:var(--accent);text-align:right;padding:0 2px;}

/* System message */
.isim-sys{
    text-align:center;font-size:11px;color:var(--txt3);
    padding:6px 12px;background:var(--bg3);
    border-radius:10px;margin:4px auto;max-width:250px;
}

/* Typing indicator */
.isim-typing{
    display:flex;gap:4px;padding:11px 14px;
    background:var(--bg3);border-radius:18px;border-bottom-left-radius:5px;
    width:fit-content;align-items:center;
}
.isim-typing span{
    width:7px;height:7px;border-radius:50%;
    background:var(--txt2);animation:ibounce .9s infinite;
}
...
.isim-loc-bub{
    ...
    border:0.5px solid var(--sep);background:var(--bg3);
    min-width:200px;
}
.isim-loc-map{
    height:100px;background:linear-gradient(135deg,#1a3a2a,#0d2218);
    display:flex;align-items:center;justify-content:center;
    font-size:32px;position:relative;overflow:hidden;
}
.isim-loc-map::before{
    content:'';position:absolute;inset:0;
    background:repeating-linear-gradient(0deg,transparent,transparent 18px,rgba(0,186,124,.08) 18px,rgba(0,186,124,.08) 19px),
               repeating-linear-gradient(90deg,transparent,transparent 18px,rgba(0,186,124,.08) 18px,rgba(0,186,124,.08) 19px);
}
.isim-loc-pin{
    font-size:28px;filter:drop-shadow(0 2px 4px rgba(0,0,0,.5));
    position:relative;z-index:1;
}
.isim-loc-label{padding:8px 12px;font-size:13px;font-weight:500;color:var(--txt);}
...
.isim-redenv-icon{font-size:34px;}
.isim-redenv-note{font-size:13px;color:rgba(255,255,255,.85);text-align:center;}
.isim-redenv-amt{font-size:20px;font-weight:700;color:#ffd700;}
...
.isim-voice-wave{flex:1;display:flex;align-items:center;gap:2px;height:24px;}
.isim-voice-wave span{
    width:3px;border-radius:2px;background:var(--txt2);
    animation:vwave 1.2s ease-in-out infinite;
}
.isim-voice-wave span:nth-child(1){height:8px;}
.isim-voice-wave span:nth-child(2){height:16px;animation-delay:.1s;}
.isim-voice-wave span:nth-child(3){height:24px;animation-delay:.2s;}
.isim-voice-wave span:nth-child(4){height:16px;animation-delay:.3s;}
.isim-voice-wave span:nth-child(5){height:8px;animation-delay:.4s;}
.isim-voice-wave span:nth-child(6){height:20px;animation-delay:.15s;}
.isim-voice-wave span:nth-child(7){height:12px;animation-delay:.25s;}
@keyframes vwave{0%,100%{transform:scaleY(.4)}50%{transform:scaleY(1)}}
.isim-voice-dur{font-size:12px;color:var(--txt3);flex-shrink:0;}

/* Photo message */
.isim-photo-bub{
    border-radius:16px;overflow:hidden;max-width:220px;cursor:pointer;
    border:0.5px solid var(--sep);
}
.isim-photo-bub img{width:100%;height:auto;display:block;max-height:280px;object-fit:cover;}

/* Sticker */
.isim-sticker-bub img{
    width:110px;height:110px;object-fit:contain;
    filter:drop-shadow(0 2px 8px rgba(0,0,0,.3));
}
.isim-sticker-emoji{font-size:56px;line-height:1;}

/* Note card */
.isim-note-card{
    background:linear-gradient(135deg,#2c2c2e,#1c1c1e);
    border:0.5px solid var(--sep);border-radius:16px;
    padding:12px 14px;min-width:180px;cursor:pointer;
}
.isim-note-head{font-size:10px;color:var(--txt3);margin-bottom:4px;text-transform:uppercase;letter-spacing:.5px;}
.isim-note-text{font-size:14px;color:var(--txt);line-height:1.4;}
.isim-note-exp{font-size:10px;color:var(--txt3);margin-top:6px;}

/* Message actions (long-press menu) */
.isim-msg-menu{
    position:absolute;bottom:calc(100% + 4px);
    background:var(--bg2);border:0.5px solid var(--sep);
    border-radius:14px;padding:4px;z-index:50;
    display:none;box-shadow:0 8px 32px rgba(0,0,0,.5);
    flex-direction:row;gap:0;overflow:hidden;min-width:200px;
}
.isim-msg-menu.show{display:flex;}
.isim-row.out .isim-msg-menu{right:0;}
.isim-row.in .isim-msg-menu{left:0;}
.isim-menu-btn{
    flex:1;background:none;border:none;
    padding:9px 8px;font-size:11px;color:var(--txt);cursor:pointer;
    display:flex;flex-direction:column;align-items:center;gap:3px;
    transition:background .1s;
}
.isim-menu-btn:active{background:var(--bg3);}
.isim-menu-btn span:first-child{font-size:18px;}
.isim-menu-btn.red{color:var(--red);}

/* Reaction bar */
.isim-react-bar{display:flex;gap:2px;margin-top:2px;}
.isim-react{
    background:var(--bg3);border:0.5px solid var(--sep);
    border-radius:10px;padding:2px 6px;font-size:13px;cursor:pointer;
    display:flex;align-items:center;gap:3px;
}
.isim-react span{font-size:11px;color:var(--txt2);}
...
.isim-inputbar-row{display:flex;gap:6px;align-items:flex-end;}
#isim-input{
...
#isim-input:focus{border-color:rgba(255,255,255,.2);outline:none;}
...
.isim-inp-btn{
    ...
    display:flex;align-items:center;justify-content:center;
    font-size:16px;cursor:pointer;flex-shrink:0;color:var(--txt2);
    transition:background .12s,transform .1s;
}
.isim-inp-btn:active{transform:scale(.9);}
#isim-plusbtn{background:var(--bg3);color:var(--txt2);}
#isim-sendbtn{
    background:var(--accent);color:#fff;
    box-shadow:0 2px 8px rgba(29,155,240,.4);
}
#isim-cancelbtn{background:#e53935;color:#fff;display:none;}
#isim-botsendbtn{background:var(--bg3);color:var(--txt2);}

/* Plus sheet — grid of quick actions */
#isim-plussheet{
...
#isim-sticker-tray{
    ...
    flex-direction:column;max-height:240px;
}
...
.isim-sticker-head-title{font-size:13px;font-weight:600;color:var(--txt);}
.isim-sticker-head-add{
...
.isim-sticker-slot{
    ...
    display:flex;align-items:center;justify-content:center;
    transition:transform .1s;border:0.5px solid var(--sep);
}
...
.isim-sticker-del{
    ...
    background:rgba(229,57,53,.9);border-radius:50%;border:none;
    color:#fff;font-size:10px;cursor:pointer;
    display:none;align-items:center;justify-content:center;
}
.edit-active .isim-sticker-del{display:flex;}
.isim-sticker-add-slot{
    aspect-ratio:1;border-radius:12px;
    background:var(--bg3);cursor:pointer;
    display:flex;align-items:center;justify-content:center;
    font-size:24px;color:var(--txt3);
    border:1.5px dashed var(--sep);
    transition:border-color .15s;
}
.isim-sticker-add-slot:active{border-color:var(--accent);}
...
.isim-redenv-card{
    background:linear-gradient(160deg,#d32f2f,#8b0000);
    border-radius:24px;padding:28px;width:100%;max-width:280px;
    display:flex;flex-direction:column;align-items:center;gap:14px;
    box-shadow:0 12px 40px rgba(180,0,0,.5);
    border:1px solid rgba(255,200,100,.2);
}
.isim-re-icon{font-size:48px;filter:drop-shadow(0 2px 6px rgba(0,0,0,.3));}
.isim-re-title{font-size:18px;font-weight:700;color:#ffd700;}
.isim-re-input{
    width:100%;background:rgba(255,255,255,.1);border:0.5px solid rgba(255,255,255,.2);
    border-radius:12px;padding:10px 14px;color:#fff;font-size:16px;text-align:center;
}
.isim-re-input::placeholder{color:rgba(255,255,255,.4);}
.isim-re-note{
    width:100%;background:rgba(255,255,255,.08);border:0.5px solid rgba(255,255,255,.15);
    border-radius:12px;padding:10px 14px;color:#fff;font-size:14px;
}
.isim-re-note::placeholder{color:rgba(255,255,255,.3);}
.isim-re-send{
    width:100%;background:#ffd700;color:#8b0000;border:none;
    border-radius:14px;padding:13px;font-size:16px;font-weight:700;cursor:pointer;
}

/* ============================================
   MESSAGE EDIT MODE
   ============================================ */
.isim-row.edit-mode .isim-bub{
    outline:1.5px solid var(--accent);border-radius:18px;
}
.isim-del-btn{
    background:#e53935;border:none;border-radius:50%;
    width:22px;height:22px;color:#fff;font-size:13px;cursor:pointer;
    align-self:flex-start;flex-shrink:0;margin-top:4px;
    display:none;align-items:center;justify-content:center;
}
...
#isim-bot-note-bubble{
    ...
    border-radius:10px;padding:4px 10px;font-size:11px;color:var(--txt2);
    max-width:150px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;
    display:none;backdrop-filter:blur(8px);
}

/* Chat screen top tools */
#isim-chat-settings{border-bottom:0.5px solid var(--sep);padding:10px 14px;}
#isim-notes-bar::-webkit-scrollbar{display:none;}
#isim-notes-bar{scroll-behavior:smooth;}
...
async function botReply() {
    ...
    try {
        ...
        // ── Build context and generate via ST pipeline ──
        ...
        const storyCtx = (s.stories||[]).filter(st=>st.isMe&&Date.now()-st.timestamp<86400000).map(st=>st.imageMeaning?`${playerName}'s story: "${st.imageMeaning}"`:null).filter(Boolean).join('. ');

        const contextNote = [
            `[iPhone Messages Context]`,
            `${displayName} is texting ${playerName} on iPhone Messages right now.`,
            userNote ? `About ${playerName}: "${userNote}"` : null,
            validBotNote ? `${displayName}'s note: "${validBotNote}"` : null,
            storyCtx || null,
            histText ? `\nRecent conversation:\n${histText}` : null,
            ``,
            `INSTRUCTIONS: Reply as ${displayName} in character. 1-3 short texts. Match language. No asterisks. No name prefix.`,
            stickerList ? `Optional: ${stickerList} — use [STICKER:N] on separate line` : null,
            `Optional: [STICKER_EMOJI:emoji]  [LOCATION:place]  [REDENV:amount:msg]  [VOICE:secs]  [NOTE:text]`,
        ].filter(x=>x!==null).join('\n');

        try { if(ctx && typeof ctx.setExtensionPrompt==='function') ctx.setExtensionPrompt('iphone-sim',contextNote,1,0,false,1); } catch(_) {}

        let rawReply = '';
        if (typeof ctx.generateQuietPrompt === 'function') {
            rawReply = await ctx.generateQuietPrompt('', false, false);
        } else if (typeof window.generateQuietPrompt === 'function') {
            rawReply = await window.generateQuietPrompt('', false, false);
        } else { throw new Error('generateQuietPrompt not available'); }

        try { if(typeof ctx.setExtensionPrompt==='function') ctx.setExtensionPrompt('iphone-sim','',1,0,false,1); } catch(_) {}

        // ── Clean reply ──
        rawReply = cleanThink(rawReply).trim();
        const _nRx = new RegExp('^' + displayName.replace(/[.*+?^${}()|[\]\\]/g,'\\$&') + '\\s*:\\s*','gim');
        rawReply = rawReply.replace(_nRx,'').replace(/^[\u201c"'\u2018]|[\u201d"'\u2019]$/g,'').trim();

        // Handle [NOTE:] commands
        for(const nm of [...rawReply.matchAll(/\[NOTE:([^\]]+)\]/gi)]) {
            cfg().botnotes[f.id]={text:nm[1].trim(),timestamp:Date.now()};
            save();updateBotNoteBubble();renderNotesBar();toast(`${displayName} posted a note`);
        }
        rawReply = rawReply.replace(/\[NOTE:[^\]]+\]/gi,'').trim();

        // ── Parse output ── accept plain text + special commands
        const rawLines = rawReply.split(/\n/).map(l=>l.trim().replace(/^\[MSG\]\s*/i,'').replace(_nRx,'').replace(/^[\u201c"'\u2018]|[\u201d"'\u2019]$/g,'').trim()).filter(l=>l);
        let parts = [];
        for (const line of rawLines) {
            if(/^\[STICKER:(\d+)\]/i.test(line)){const si=parseInt(line.match(/\d+/)[0]);const stk=(s.stickers||[])[si];parts.push(stk?{type:'sticker_img',src:stk.src}:{type:'sticker_emoji',content:'\u2728'});}
            else if(/^\[STICKER_EMOJI:(.+)\]/i.test(line)){parts.push({type:'sticker_emoji',content:line.match(/^\[STICKER_EMOJI:(.+)\]/i)[1]});}
        ...
        if(!parts.length&&rawReply){
            const cleaned=rawReply.replace(/\[[A-Z_]+:[^\]]*\]/gi,'').trim();
            if(cleaned){const sents=cleaned.match(/[^\n.!?]+[\n.!?]?/g)||[cleaned];sents.slice(0,3).forEach(s=>{const t=s.trim();if(t)parts.push({type:'text',content:t});});}
        }
        if(!parts.length)parts.push({type:'text',content:rawReply.split('\n')[0].trim()||'...'});
...
window.__isimCallSend = async function() {
    ...
    try {
        ...
        const callCtx = SillyTavern.getContext();
        const callHist = (cfg().history[activeFriend.id]||[]).slice(-6).map(m=>(m.from==='user'?getUserName():activeFriend.name)+': '+(m.content||'')).join('\n');
        const callNote = [`[Phone Call]`,`${activeFriend.name} is on a phone call with ${getUserName()}.`,callHist?`Recent chat:\n${callHist}`:'',`${getUserName()} just said: "${text}"`,`Reply as ${activeFriend.name} speaking on the phone. 1-2 natural sentences. Same language. No tags.`].filter(Boolean).join('\n');
        try{if(typeof callCtx.setExtensionPrompt==='function')callCtx.setExtensionPrompt('iphone-sim-call',callNote,1,0,false,1);}catch(_){}
        let reply = '';
        if(typeof callCtx.generateQuietPrompt==='function'){reply=await callCtx.generateQuietPrompt('',false,false);}
        else if(typeof window.generateQuietPrompt==='function'){reply=await window.generateQuietPrompt('',false,false);}
        try{if(typeof callCtx.setExtensionPrompt==='function')callCtx.setExtensionPrompt('iphone-sim-call','',1,0,false,1);}catch(_){}
        reply = cleanThink(String(reply||'').trim());
