...
// ── Image processing (no dynamic import) ──
let _mediaCfg = { imageQuality: 85, maxImageDimension: 2048 };

function _processImageDataURL(dataURL, callback) {
    const img = new Image();
    img.onload = function() {
        const canvas = document.createElement('canvas');
        let { width, height } = img;
        const maxD = _mediaCfg.maxImageDimension;
        if (width > maxD || height > maxD) {
            if (width >= height) { height = Math.round(height * maxD / width); width = maxD; }
            else { width = Math.round(width * maxD / height); height = maxD; }
        }
        canvas.width = width; canvas.height = height;
        canvas.getContext('2d').drawImage(img, 0, 0, width, height);
        callback(canvas.toDataURL('image/jpeg', _mediaCfg.imageQuality / 100));
    };
